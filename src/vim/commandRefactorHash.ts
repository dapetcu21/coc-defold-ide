...
function applyEdits(document: Document, edits: Edit[]): TextEdit[] {
  ...
  edits.forEach((edit) => {
    switch (edit.type) {
      case "insert":
        textEdits.push(
          TextEdit.insert(textDocument.positionAt(edit.offset), edit.text)
        );
        break;
      case "replace":
        textEdits.push(
...
export default function registerRefactorHashCommand(context: ExtensionContext) {
  context.subscriptions.push(
    commands.registerCommand("defold-ide.refactorHash", async function () {
      await refactorHash(context, false);
    })
  );
  ...
  context.subscriptions.push(
    commands.registerCommand(
      ...
      async function () {
        await refactorHash(context, true);
      }
    )
  );
}
